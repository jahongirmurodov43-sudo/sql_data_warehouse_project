...
SELECT 
	ROW_NUMBER() OVER (ORDER BY pn.prd_start,pn.pdr_key) AS product_key,
	pn.prd_id                                            AS product_id,
	pn.pdr_key                                           AS product_number,
	pn.prd_mm                                            AS product_name,
...
LEFT JOIN silver.erp_px_cat_g1v AS pc
ON		  pn.cat_id = pc.id
WHERE pn.prd_end IS NULL -- Filter out all historical data 
...
SELECT 
	sd.sld_ord_num              AS order_number,
	pr.product_key,
	cu.customer_key,
